...
module.exports = {
    name: "deaths",
	description: "Xem 5 lần chết gần nhất",
	delay: 10,
    
    async execute(client, message, args) {
        if (!args[0]) return message.lineReplyNoMention(client.inputUsername).then(msg => msg.delete({timeout: 60000}));

		var quotes = new Scriptdb(`${client.disk}/data/deaths/${args[0]}.json`)
		var deaths = quotes.get('deaths')
		var times = quotes.get('times')

		if(times == undefined || deaths == undefined) return message.lineReplyNoMention(client.userNotFound).then(msg => msg.delete({timeout: 60000}));
